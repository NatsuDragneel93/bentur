...
- `src/pages/tours/setup/` — editor Setup A/B (rotta `/tours/:tourId/artists/:artistId/setup/:setupKey`, `a`|`b`): `SetupEditor` (stato: cronologia annulla/ripristina, salvataggio, dialoghi conflitto / copia da Setup A / uscita senza salvare), `StageCanvas` (Konva: forme, maniglie, zoom con rotella/pizzico, spostamento vista, `exportImage` PNG), `ShapePalette`, `ShapeProperties` (nome, colori, duplica, ordine; su cellulare pannello dal basso). Logica pura in `src/utils/stagePlot.ts` (palco logico 1000×600, coordinate = centro della forma, zoom/vista) e `src/utils/history.ts`. Servizio `artistSetups.service.ts` (`saveSetup` in transazione: `conflict` se `updatedAt` è cambiato dal caricamento, `force` per sovrascrivere). Nei test vanno mockati `react-konva` e `konva` (jsdom non ha canvas). Con `BrowserRouter` non si può bloccare la navigazione: l'avviso di modifiche non salvate copre solo il pulsante indietro e `beforeunload`
