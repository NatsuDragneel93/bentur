...
- `src/pages/tours/setup/` — editor Setup A/B (rotta `/tours/:tourId/artists/:artistId/setup/:setupKey`, `a`|`b`): `SetupEditor` (stato), `StageCanvas` (Konva), `ShapePalette`. Logica pura in `src/utils/stagePlot.ts` (palco logico 1000×600, coordinate = centro della forma). Nei test `react-konva` va mockato (jsdom non ha canvas). Servizio `artistSetups.service.ts` (`getSetup`, `saveSetup` in transazione: restituisce `conflict` se `updatedAt` è cambiato dal caricamento, `force` per sovrascrivere). In sviluppo: l'editor non usa ancora il servizio
